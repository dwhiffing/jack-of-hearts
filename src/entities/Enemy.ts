...
export class Enemy extends EntityBase {
  ...
  constructor(scene: Game, x: number, y: number) {
    super(scene, x, y, 2)
    this.setSize(8, 4).setScale(3).setCollideWorldBounds(false)
  }
  ...
  public moveTowards(target: Core): void {
    ...
    if (this.active && this.body && this.sceneRef.physics && target) {
      ...
      if (dist < attackDist) {
        // stop moving and wait to attack
        ...
        // dont move if waiting for attack cooldown
